...
int ContextRun(struct camera_stream_t *pstr)
{
    ...
    for (i = 0; i < pstr->NrFrame; i++)
    {
        ...
        memset(&cam_ufds,0,sizeof(cam_ufds));
        cam_ufds.fd = pstr->fd_cam;
        cam_ufds.events = POLLIN;
        ret = poll(&cam_ufds, 1, V4L2_POLL_TIMEOUT_MS);
        if (ret>0)
        {
            c_info("%s poll successed, ret= %d, revents= %d",pstr->name,ret,cam_ufds.revents);
            goto LABEL_DQBUF;
        }
        if (!ret)
            c_info("%s poll timeout no data",pstr->name);
        if (ret<0)
            c_info("%s poll error %s",pstr->name,strerror(errno));
        continue;
        //goto LABEL_STREAM_OFF;
LABEL_DQBUF:
