...
{
    int i,j;
    for( i=0; i<ARRAY_SIZE(idm_map_table); i++ ){
        if(idm_map_table[i].id == node->id){
            node->type = idm_map_table[i].type;
            return 0;
        }
    }
    return -1;
...
{
    if (node->open_flag == 0){
        node->fd = idm_open(node->me->devname, O_RDWR | O_NONBLOCK);
        if(node->fd < 0){
...
int idm_ctl_init(void)
{
    ...
    for (i = 0; i < IDM_NODE_MAX_CNT; i++) {
        ...
        if(0 > idm_ctl_node_add(&idm_plat, me, idname_map->id))
            goto err_out;
    }
    return 0;
...
/*
   ...
   {
   return idm_ioctl(fd, VIDIOC_SUBDEV_S_FMT, &fmt);
   }
...
int idm_ctl_set_fmts(struct idm_fmt_crop *fmts, int n)
{
    ...
    for(i=0; i<n; i++){
        ...
        if(fmt->ops & IDM_APPLY_FMT){
            ...
            ret = idm_ioctl(fd, VIDIOC_SUBDEV_S_FMT, &format);
            if(ret < 0) {
        ...
        if(fmt->ops & IDM_APPLY_CROP){
            ...
            ret = idm_ioctl(fd, VIDIOC_SUBDEV_S_SELECTION, &selection);
            if(ret < 0) {
